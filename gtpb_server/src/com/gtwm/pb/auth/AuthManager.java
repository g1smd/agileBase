...
public CompanyInfo getCompanyForLoggedInUser(HttpServletRequest request)
        throws ObjectNotFoundException {
    AppUserInfo loggedInUser = ((Authenticator) this.authenticator).getUserByUserName(request.getRemoteUser());
...
public synchronized void addUser(HttpServletRequest request, AppUserInfo newUser)
    ...
    if (!(this.authenticator.loggedInUserAllowedTo(request, PrivilegeType.ADMINISTRATE) || this.authenticator.loggedInUserAllowedTo(request, PrivilegeType.MASTER))) {
...
public synchronized void removeRolePrivilege(HttpServletRequest request, AppRoleInfo role,
    ...
    HibernateUtil.activateObject(this.authenticator);
    RoleGeneralPrivilegeInfo removedPrivilege = ((Authenticator) this.authenticator).removeRolePrivilege(role, privilegeType);
...
public synchronized void removeRolePrivilege(HttpServletRequest request, AppRoleInfo role,
    ...
    HibernateUtil.activateObject(this.authenticator);
    RoleObjectPrivilegeInfo removedPrivilege = ((Authenticator) this.authenticator).removeRolePrivilege(role, privilegeType, table);
    HibernateUtil.currentSession().delete(removedPrivilege);
}
...
public synchronized void removeUserPrivilege(HttpServletRequest request, AppUserInfo appUser,
    ...
    HibernateUtil.activateObject(this.authenticator);
    UserGeneralPrivilegeInfo removedPrivilege = ((Authenticator) this.authenticator).removeUserPrivilege(appUser, privilegeType);
    HibernateUtil.currentSession().delete(removedPrivilege);
}
...
public synchronized void removeUserPrivilege(HttpServletRequest request, AppUserInfo appUser,
    ...
    HibernateUtil.activateObject(this.authenticator);
    UserObjectPrivilegeInfo removedPrivilege = ((Authenticator) this.authenticator).removeUserPrivilege(appUser, privilegeType, table);
    HibernateUtil.currentSession().delete(removedPrivilege);
}
...
public void removePrivilegesOnTable(HttpServletRequest request, TableInfo table)
    ...
    HibernateUtil.activateObject(this.authenticator);
    Set<UserGeneralPrivilegeInfo> allUserPrivileges = ((Authenticator) this.authenticator).getUserPrivileges();
    ...
    Set<RoleGeneralPrivilegeInfo> allRolePrivileges = ((Authenticator) this.authenticator).getRolePrivileges();
...
public boolean specifiedUserHasPrivilege(HttpServletRequest request,
    ...
    for (UserGeneralPrivilegeInfo privilege : userPrivileges) {
        ...
        if (privilege instanceof UserObjectPrivilegeInfo) {
            if (privilege.getPrivilegeType().equals(privilegeType)
                    && ((UserObjectPrivilegeInfo) privilege).getTable().getInternalTableName().equals(table.getInternalTableName())) {
                return true;
            }
        }
    }
...
public boolean specifiedRoleHasPrivilege(HttpServletRequest request,
    ...
    for (RoleGeneralPrivilegeInfo privilege : rolePrivileges) {
        if (privilege instanceof RoleObjectPrivilegeInfo) {
            if (privilege.getPrivilegeType().equals(privilegeType)
                    && ((RoleObjectPrivilegeInfo) privilege).getTable().getInternalTableName().equals(table.getInternalTableName())) {
                return true;
            }
        }
    }
