...
public class Company implements CompanyInfo, Comparable<CompanyInfo> {
    ...
    @OneToMany(mappedBy="company", targetEntity=AppUser.class, cascade=CascadeType.ALL) // Bi-directional OneToMany
    ...
    public SortedSet<AppRoleInfo> getRoles() {
        return Collections.unmodifiableSortedSet(new TreeSet<AppRoleInfo>(this.getRolesCollection()));
    }
    
    @OneToMany(mappedBy="company", targetEntity=AppRole.class, cascade=CascadeType.ALL) // Bi-directional OneToMany
    ...
    public SortedSet<ModuleInfo> getModules() {
    	return Collections.unmodifiableSortedSet(new TreeSet<ModuleInfo>(this.getModulesCollection()));
    }
    ...
    public ModuleInfo getModuleByInternalName(String internalModuleName) throws ObjectNotFoundException {
    	...
    	throw new ObjectNotFoundException("Can't find module with internal name " + internalModuleName + " in company " + this);
    }
    ...
    public Set<String> getTabAddresses() {
    	return Collections.unmodifiableSet(new LinkedHashSet<String>(this.getTabAddressesCollection()));
    }
	...
	public int compareTo(CompanyInfo otherCompany) {
        ...
        String otherCompareString = otherCompany.getCompanyName().toLowerCase() + otherCompany.getInternalCompanyName();
