...
public final class CalendarPublisher extends HttpServlet {
	...
	/**
	 * This is synchronized to stop memory spikes. MS Outlook fetches calendars
	 * simultaneously, so if there are a dozen calendars or so there can be a
	 * bit of a spike. It's not necessary to return all of them super-quickly
	 * into Outlook anyway
	 */
			...
			for (DataRowInfo reportDataRow : reportDataRows) {
				...
				if (wholeDay) {
					...
					if (eventEndField.equals(eventStartField)) {
						...
						eventEndDate = new net.fortuna.ical4j.model.Date(endEpochTime);
